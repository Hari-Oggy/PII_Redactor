...
func NewHandler(
	...
	wsProxy := NewWebSocketProxy(pipeline, redactor)
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if websocket.IsWebSocketUpgrade(r) {
		upstreamURL := StreamReqToURL(r, providerCfg.BaseURL, providerCfg.PathPrefix)
		uctx := middleware.GetUserContext(r)
		h.wsProxy.Proxy(w, r, upstreamURL, tokenMap, uctx, requestID)
		return
	}
	...
	for key, text := range fields {
		...
		if len(matches) > 0 {
			uctx := middleware.GetUserContext(r)
			redacted, spans := h.redactor.Redact(text, matches, tokenMap, uctx)
	...
	if strings.Contains(contentType, "text/event-stream") {
		...
		if err := h.streaming.ProxySSE(w, upstreamResp.Body, tokenMap, middleware.GetUserContext(r), requestID); err != nil {
	...
	if err == nil && len(respFields) > 0 {
		...
		for key, text := range respFields {
			...
			if len(matches) > 0 {
				uctx := middleware.GetUserContext(r)
				redacted, _ := h.redactor.Redact(text, matches, tokenMap, uctx)
				text = redacted
			}
